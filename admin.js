(async function () {
  ...
  document.getElementById("add-file").onclick = async () => {
    ...
    const path = `uploads/archive/${Date.now()}-${safeName}`;
    const reader = new FileReader();
    reader.onload = async () => {
      try {
        status.textContent = "Uploading file to GitHub…";
        await putFile(path, reader.result.split(",")[1], `Upload archive file ${safeName}`);
        ...
        render();
        status.textContent = "File uploaded — publish to add it to the site.";
      } catch (error) {
